<x-admin-layout>
    <x-slot name="title">Edit Songket</x-slot>

    <div class="max-w-4xl mx-auto">
        <div class="bg-white shadow-sm rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-900">Edit Songket: {{ $songket->name }}</h2>
                <p class="mt-1 text-sm text-gray-600">Perbarui informasi produk songket.</p>
            </div>

            <form action="{{ route('admin.songket.update', $songket) }}" method="POST" enctype="multipart/form-data"
                class="p-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-6">
                            ...
                            <select id="category_id" name="category_id"
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ old('category_id', $songket->category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                                :value="old('name', $songket->name)" required />
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-amber-500 focus:ring-amber-500" required>{{ old('description', $songket->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <x-text-input id="base_price" name="base_price" type="number" min="1000" step="1000"
                                class="mt-1 block w-full" :value="old('base_price', $songket->base_price)" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="stock_quantity" name="stock_quantity" type="number" min="0"
                                class="mt-1 block w-full" :value="old('stock_quantity', $songket->stock_quantity)" required />
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div id="colors-container" class="mt-1 space-y-2">
                                @php
                                    $colors = old('colors', $songket->colors ?? []);
                                    if (empty($colors)) {
                                        $colors = [''];
                                    }
                                @endphp
                                @foreach ($colors as $index => $color)
                                    <div class="flex items-center space-x-2 color-input">
                                        <x-text-input name="colors[]" type="color" class="flex-1" :value="$color"
                        ...
                        @php
                            $existingImages = collect($songket->images ?? [])
                        ...
                        <x-image-upload name="images" label="Gambar Produk" :multiple="true" :max-files="5"
                            :required="true" :max-size="2048" :existing-images="$existingImages"
                            help="Format: JPEG, PNG, JPG, WebP. Maksimal 5 gambar, masing-masing 2MB. Gambar baru akan ditambahkan ke gambar yang sudah ada." />
                            ...
                            <div>
                                <label class="flex items-center">
                                    <input type="checkbox" name="is_featured" value="1"
                                        {{ old('is_featured', $songket->is_featured) ? 'checked' : '' }}
                            ...
                            <div>
                                <label class="flex items-center">
                                    <input type="checkbox" name="is_active" value="1"
                                        {{ old('is_active', $songket->is_active) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-amber-600 shadow-sm focus:ring-amber-500" />
